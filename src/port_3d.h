...
public:
	...
	const Port_3D &operator =(const Port_3D &);
	void copy(Port_3D &);
...
inline void Port_3D::calc_rho()
{
  ...
  slook_from.rho = static_cast<REAL_TYPE>(sqrt((dX*dX) + (dY*dY) + (dZ*dZ)));
}
...
inline void Port_3D::port2screen(const R_3DPoint &tr, R_2DPoint *rp)
{
  ...
  REAL_TYPE z_inv;
  if (tr.z >= 1.0)
    z_inv = static_cast<REAL_TYPE>(1.0 / tr.z);
  else
    z_inv = 1.0;
...
inline void Port_3D::port2screen(const TR_3DPoint &tr, TR_2DPoint *rp, int persp)
{
  ...
  REAL_TYPE z_inv;
  if (tr.r.z >= 1.0)
    z_inv = static_cast<REAL_TYPE>(1.0 / tr.r.z);
  else
    z_inv = 1.0;
...
{
  sin_theta = static_cast<REAL_TYPE>(sin(slook_from.theta));
  sin_phi =   static_cast<REAL_TYPE>(sin(slook_from.phi)  );
  cos_theta = static_cast<REAL_TYPE>(cos(slook_from.theta));
  cos_phi =   static_cast<REAL_TYPE>(cos(slook_from.phi)  );
}
...
{
  slook_from.theta = th;
  sin_theta = static_cast<REAL_TYPE>(sin(th));
  cos_theta = static_cast<REAL_TYPE>(cos(th));
  calc_look_at();
}
...
{
  slook_from.phi = ph;
  sin_phi = static_cast<REAL_TYPE>(sin(ph));
  cos_phi = static_cast<REAL_TYPE>(cos(ph));
  calc_look_at();
}

inline void Port_3D::set_roll(REAL_TYPE rll)
{
  roll = limit_angle(rll);
  sin_roll = static_cast<REAL_TYPE>(sin(rll));
  cos_roll = static_cast<REAL_TYPE>(cos(rll));
}
