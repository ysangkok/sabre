...
public:
	...
	bool select_next_target(unsigned int);
	void set_view_node(int);
	Flight_Node *get_node(unsigned int n)
	{
		if (static_cast<unsigned int>(n) < n_flights)
	...
	Flight *get_flight(unsigned int i)
	{
		if (i < n_flights)
	...
	Flight_ZViewer *get_viewer(unsigned int i)
	{
		if (i < n_flights)
